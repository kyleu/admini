...
##
## Outputs:
##   - build/dist/admini_<version>_darwin_*_desktop.(dmg|zip)
##   - build/dist/admini_<version>_linux_amd64_desktop.zip
##   - build/dist/admini_<version>_windows_amd64_desktop.zip
...
then
  retry -t 4 -- docker build -f tools/desktop/Dockerfile.desktop -t admini .
else
...
cd "$dir/../.."
echo "Builds written to ./build/dist (admini_${TGT}_*_desktop.*)"
