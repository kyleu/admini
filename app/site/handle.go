...
func Handle(as *app.State, path []string, ps *cutil.PageState) (string, layout.Page, []string, error) {
	...
	case util.AppKey:
		msg := "\n  " +
			...
			"<meta name=\"go-source\" content=\"admini.dev/admini %s %s/tree/main{/dir} %s/blob/main{/dir}/{file}#L{line}\">"
	...
	case keyDownload:
		...
		data := util.ValueMap{"base": "https://admini.dev/admini/releases/download/v" + as.BuildInfo.Version, "links": dls}
