...
func typeFor(t string, cr *columnResult, enums model.Models, logger util.Logger) *types.Wrapped {
	if cr != nil && cr.Nullable == sqlServerYes {
		cr.Nullable = sqlServerNo
	...
	case "float8", "double precision", "double", "decimal":
	...
	if e := enums.Get(util.Pkg{cr.Schema}, t); e != nil {
		return types.NewEnum(cr.Name)
	}
	logger.Warn("unhandled SQL Server type: " + t)
